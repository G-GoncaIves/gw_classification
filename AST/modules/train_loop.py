...
def train(audio_model, train_loader, val_loader, train_conf):
    ...
    best_epoch, best_mAP, best_acc = 0, 0, -np.inf
    global_step = 0
    ...
    if verbose:
        epoch_progress_bar = tqdm(range(1, train_conf["epochs"]+1), desc="Trainig", ncols=90, leave=False, position=0, bar_format="{l_bar}{bar}| eta: {eta}", colour="magenta")
        batch_progress_bar = enumerate(tqdm(train_loader, desc="Batch", ncols=90, leave=False, position=1, bar_format="{l_bar}{bar}| {remaining}", colour="cyan"))

    else:
        epoch_progress_bar = range(1, train_conf["epochs"]+1)
        batch_progress_bar = enumerate(train_loader)    
    ...
    for epoch in epoch_progress_bar:
        ...
        for i, (audio_input, labels) in batch_progress_bar:
        ...
        #
        stats, valid_loss = validate(audio_model, val_loader, train_conf, epoch, verbose)
...
def validate(audio_model, val_loader, train_conf, epoch, verbose):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    batch_time = AverageMeter()
    if not isinstance(audio_model, nn.DataParallel):
        audio_model = nn.DataParallel(audio_model)
    ...
    if verbose:
        batch_progress_bar = enumerate(tqdm(val_loader, desc="Validating", ncols=90, leave=False, position=1, bar_format="{l_bar}{bar}| {remaining}", colour="cyan"))
    else:
        batch_progress_bar = enumerate(val_loader)
